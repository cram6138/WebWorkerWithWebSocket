...
var CACHE_NAME = "spmTrackerCache_";
...
const pullData = async (eventData) => {
  ...
  const cache = await caches.open(CACHE_NAME + tracker_Id);
  ...
  try {
    ...
    return Promise.all(
      cacheKeys.map(cacheKey => {
        if (cacheKey === (CACHE_NAME + tracker_Id)) return caches.delete(cacheKey);
      })
    );
  } catch (error) {
...
onmessage = async (event) => {
  ...
  const { url, trackerId, page, pageSize, sort, filter } = event.data;
